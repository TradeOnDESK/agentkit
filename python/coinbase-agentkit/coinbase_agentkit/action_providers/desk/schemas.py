...
class GetHistoricalFundingRatesSchema(BaseModel):
    ...
    symbol: str = Field(..., description="The symbol of the asset to get historical funding rates for")
    start_time: int = Field(..., description="The start time (in seconds) to get historical funding rates for")
    end_time: int = Field(..., description="The end time (in seconds) to get historical funding rates for")
...
class PlaceOrderSchema(BaseModel):
    ...
    side: str = Field(...,
                      description=f"The side of the order ({[e.value for e in OrderSide]})")
...
class CancelOrderSchema(BaseModel):
    ...
    is_conditional_order: bool = Field(...,
                                       description="Whether this is a conditional order")
    order_digest: str | None = Field(
        None, description="The order digest to cancel")
    wait_for_reply: bool = Field(
        True, description="Whether to wait for reply from the server")
    client_order_id: str | None = Field(
        None, description="Client order ID for tracking")
...
class CancelAllOrdersSchema(BaseModel):
    ...
    is_conditional_order: bool = Field(
        False, description="Whether to cancel conditional orders")
    wait_for_reply: bool = Field(
        True, description="Whether to wait for reply from the server")
